...
**Where:** Knowledge library §`ops` > "Stabilised Approach Gate": *"JAK/I-Fly: stabilised by 1,000 ft AAL IMC / 500 ft AAL VMC."*
...
**Where:** CLAUDE.md flags this as safety-significant: *"TOCWS does NOT alert for Flaps 0 (valid config) — EICAS confirmation discipline mandatory."*
...
Current prompt: *"Each question must have 4 options."* — no instruction to make distractors plausible, no instruction to avoid trick wording, no instruction to vary cognitive level (recall vs application vs analysis per Bloom).
...
— *Audit prepared for Capt. Dan Moi Ng'ong'a, DN Consultancy Aviation, against the production rebuild specification in `CLAUDE.md` and `README.md`.*
